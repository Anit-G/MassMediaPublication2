...
from Utils.DB_Operations import Status
...
class UploadWorker(BaseWorker):
    ...
    def get_next_item(self):
        retries = self.uploader._dbops.get_stage_retries(Status.PossibleStates.UPLOAD)
        category_retries = [r for r in retries if len(r) >= 4 and r[3] == self.category]
        if category_retries:
            return ("retry", category_retries[0])

        res = self.uploader._dbops.get_next_action_category(self.category, Status.ProcessingStage.PROCESSING, Status.PossibleStates.UPLOAD)
        if res is not None:
            return ("normal", res)
        return None
